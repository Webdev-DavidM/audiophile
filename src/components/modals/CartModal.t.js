/*

1.When the user clicks the view cart button, the modal cart appears
2.The cart shows the correct amount of items in the cart
3. If the user - or + from an item  it is removed and the total is updated
4.If the user clicks remove all the empty becomes empty. 
5. Once the user is at 0 on the itmes, the - button is disabled.
6. If the user clicks checkout with something in the basket, they are taken to the checkout page
7. If the user clicks the checkout and they are not signed in they are re directed to sign up page

*/

test("test", () => {});
